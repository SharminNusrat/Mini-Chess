...
class GameSetupMenu:
    ...
    def draw(self):
        ...
        # Radio buttons for white player
        ...
        # Radio buttons for black player
    ...
    def handle_click(self, pos):
        ...
        if 65 <= x <= 105:
            if 280 <= y <= 316:  # Human player
                self.white_player = "human"
                return None
            elif 322 <= y <= 358:  # AI player
                self.white_player = "ai"
                return None
        
        # Black player selection
        if 65 <= x <= 105:
            if 455 <= y <= 491:  # Human player
                self.black_player = "human"
                return None
            elif 497 <= y <= 533:  # AI player
